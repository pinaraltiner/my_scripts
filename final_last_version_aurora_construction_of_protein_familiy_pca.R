...
hist(rd, xlab="RMSD (Å)", main="Histogram of RMSD", breaks = 72)

# RMSD clustering
hc.rd <- hclust(as.dist(rd))

pdbs_aurora$id <- substr(basename(pdbs_aurora$id), 1, 6)
hclustplot(hc.rd, labels=pdbs_aurora$id, k=3 ,cex=0.5,ylab="RMSD (Å)", main="RMSD Cluster Dendrogram", fillbox=FALSE)
...
plot.bio3d(rf2, ylab="RMSF (Å)", xlab="Residue No.", typ="l")
...
#coloring method u active ve inactive olarak düzelt.
